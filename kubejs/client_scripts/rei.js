...
onEvent('rei.group', event => {
	...
	event.groupItems(`mbm2:rei_groups/wood_constructs`, '木制构造体部件', [
		/mna:constructs.*_wood/,
	])
	event.groupItems(`mbm2:rei_groups/stone_constructs`, '石制构造体部件', [
		/mna:constructs.*_stone/,
	])
	event.groupItems(`mbm2:rei_groups/iron_constructs`, '铁制构造体部件', [
		/mna:constructs.*_iron/,
	])
	event.groupItems(`mbm2:rei_groups/gold_constructs`, '金制构造体部件', [
		/mna:constructs.*_gold/,
	])
	event.groupItems(`mbm2:rei_groups/diamond_constructs`, '钻石构造体部件', [
		/mna:constructs.*_diamond/,
	])
	...
	*/
	...
	global.newMaterialParts.forEach(item => {
		if (item.ore) {
			var groupName = item.ore_name + '矿石'
	...
	event.groupItems('mbm2:rei_groups/spawn_eggs', '刷怪蛋', [
		/spawn_egg/,
		/^ars_nouveau:.*_se$/,
